...
class WC_PB_Analytics_Stock_REST_Controller extends WC_REST_Reports_Controller implements ExportableInterface {
	 ...
	 */
	protected function prepare_links( $object_data ) {
		$links = array(
			'bundle'  => array(
				'href' => rest_url( sprintf( '/%s/%s/%d', $this->namespace, 'products', $object_data['bundle_id'] ) ),
			),
			'product' => array(
				'href' => rest_url( sprintf( '/%s/%s/%d', $this->namespace, 'products', $object_data['product_id'] ) ),
			),
		);
	 ...
	 * @return string
	 */
	protected function _get_stock_status( $status ) { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
	...
	public function get_export_columns() {
		 ...
		 *
		 * @since 6.9.0
		 * @param array $export_columns Array of column names.
		 */
	...
	public function prepare_item_for_export( $item ) {
		 ...
		 *
		 * @since 6.9.0
		 * @param array $export_item Array of column values.
		 * @param array $item        Single report item/row.
		 */
		return apply_filters(
